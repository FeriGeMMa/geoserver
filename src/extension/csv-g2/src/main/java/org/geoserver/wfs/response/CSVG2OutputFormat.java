...
public class CSVG2OutputFormat extends WFSGetFeatureOutputFormat {
  private String delimiter = ";";

  public CSVG2OutputFormat(GeoServer gs, String delimiter) {
    super(gs, "csv-g2");
  ...
  public void write(FeatureCollection<?, ?> fc, OutputStream output)
      throws IOException, ServiceException {
    // write out content here

    // create a writer
    BufferedWriter w = new BufferedWriter(new OutputStreamWriter(output, Charset.forName("UTF-8")));

    if (fc.getSchema() instanceof SimpleFeatureType) {
      // write out the header: no FID!
      ...
      for (PropertyDescriptor att : fc.getSchema().getDescriptors()) {
        ...
        if (!att.getName().getLocalPart().startsWith("FEATURE_LINK")) {
          ...
          if (xsd != null && xsd instanceof XSDElementDeclarationImpl) {
            ...
            // otherwise defaults to the full name with namespace
            // URI
    ...
    NumberFormat coordFormatter = NumberFormat.getInstance(Locale.getDefault());
      ...
      while (i.hasNext()) {
        ...
        if (f instanceof SimpleFeature) {
          ...
          while (descriptors.hasNext()) {
            ...
            j++;
            // Multi valued properties aren't supported, only for
            // SF0 for now
  ...
  private String formatToString(Object att, NumberFormat coordFormatter) {
    ...
    if (att instanceof Number) {
      // don't allow scientific notation in the output, as OpenOffice
      // won't
